...
int         CLKS_DWORD   =   0;  /* use clks/byte? (vs. clks/block) */
...
void TimeOps(int iterCnt)
    ...
    const char *testName[TEST_CNT]={"BlockEncrypt:","BlockDecrypt:","reKeyEncrypt:"};
    const char *atomName[TEST_CNT]={"block","block","call "};
    const char *format  [TEST_CNT]={"%10.1f/%s ","%10.1f/%s ","%10.1f/%s "};
    ...
    for (i=0;i<TEST_CNT;i++)
        ...
        if (needSet[i] & 1)
            {
            denom[i]=sizeof(text)/((CLKS_DWORD) ? 1 : (BLOCK_SIZE/8) );
            atomName[i] = (CLKS_DWORD) ? "byte "      : "block";
            }
        format  [i] = (CLKS_DWORD) ? "%10.1f/%s " : "%10.0f/%s ";
        }
...
void AES_Sanity_Check(int testCnt)
    ...
    const char *modeNames[]={"(null)","MODE_ECB","MODE_CBC","MODE_CFB1"};
    ...
    {        
        ...
        mName=modeNames[(size_t)mode];
        if (cipherInit(&t.ci,mode,hexString) != TF_SUCCESS)
        ...
        for (t.keySize=KEY_BITS_0;t.keySize <= MAX_KEY_BITS;t.keySize+=STEP_KEY_BITS)
            ...
            if (makeKey(&t.ki,DIR_ENCRYPT,t.keySize,hexString) != TF_SUCCESS)
            ...
            for (testNum=0;testNum<lim;testNum++)
                ...
                if ((testNum&0x1F)==0)  /* periodic re-key time? */
                    ...
                    if (makeKey(&t.ki,DIR_ENCRYPT,t.keySize,t.ki.keyMaterial) != TF_SUCCESS)
                        FatalError("Encrypt makeKey during sanity check %s",mName);
                    debug=0;
                    if (makeKey(&k2  ,DIR_DECRYPT,t.keySize,t.ki.keyMaterial) != TF_SUCCESS)
...
void AES_Test_VK(const char *fname)
{
    ...
    if (cipherInit(&t.ci,MODE_ECB,NULL) != TF_SUCCESS)
...
void AES_Test_TBL(const char *fname)
    ...
    for (t.keySize=KEY_BITS_0;t.keySize <= MAX_KEY_BITS;t.keySize+=STEP_KEY_BITS)
        ...
        for (t.I=1;TableOp(TAB_QUERY) == TF_FAILURE;t.I++)
            {
            if (makeKey(&t.ki,DIR_ENCRYPT,t.keySize,t.ki.keyMaterial) != TF_SUCCESS)
                FatalError("Error parsing key during %s test",fname);
            if (blockEncrypt(&t.ci,&t.ki,t.pt,BLOCK_SIZE,t.ct) != BLOCK_SIZE)
        ...
        for (t.I=0;t.I<mctOuter;t.I++)
            ...
            for (j=0;j<mctInner;j++)
                ...
                if (j == mctInner-1)    /* xor the key for next outer loop */
                    for (i=0;i<t.keySize/32;i++)
                        t.ki.key32[i] ^=
                            Bswap((i>=q) ? ((uint32_t *)t.ct)[i-q] :
                                           ((uint32_t *)t.pt)[BLOCK_SIZE/32-q+i]);
                BlockCopy(t.pt,t.ct);
                }
            AES_printf(&t,"C\n");
            if (reKey(&t.ki) != TF_SUCCESS)
...
void AES_Test_ECB_D_MCT(const char *fname)
    ...
    for (t.keySize=KEY_BITS_0,q=0;t.keySize <= MAX_KEY_BITS;t.keySize+=STEP_KEY_BITS,q+=2)
        ...
        ClearTestData(&t);              /* start with all zeroes */
        if (makeKey(&t.ki,DIR_DECRYPT,t.keySize,t.ki.keyMaterial) != TF_SUCCESS)
            FatalError("Error parsing key during %s test",fname);
        ...
        for (t.I=0;t.I<mctOuter;t.I++)
            ...
            for (j=0;j<mctInner;j++)
                ...
                if (j == mctInner-1)    /* xor the key for next outer loop */
                    for (i=0;i<t.keySize/32;i++)
                        t.ki.key32[i] ^=
                            Bswap((i>=q) ? ((uint32_t *)t.pt)[i-q] :
                                           ((uint32_t *)t.ct)[BLOCK_SIZE/32-q+i]);
                BlockCopy(t.ct,t.pt);
                }
            AES_printf(&t,"P\n");
            if (reKey(&t.ki) != TF_SUCCESS)
    ...
    for (t.keySize=KEY_BITS_0,q=0;t.keySize<=MAX_KEY_BITS;t.keySize+=STEP_KEY_BITS,q+=2)
        ...
        for (t.I=0;t.I<mctOuter;t.I++)
            ...
            for (i=0;i<t.keySize/32;i++)
                t.ki.key32[i] ^=
                    Bswap((i>=q) ? ((uint32_t *)t.ct  )[i-q] :
                                   ((uint32_t *)ctPrev)[BLOCK_SIZE/32-q+i]);
    ...
    for (t.keySize=KEY_BITS_0,q=0;t.keySize <= MAX_KEY_BITS;t.keySize+=STEP_KEY_BITS,q+=2)
        ...
        for (t.I=0;t.I<mctOuter;t.I++)
            ...
            for (i=0;i<t.keySize/32;i++)
                t.ki.key32[i] ^=
                    Bswap((i>=q) ? ((uint32_t *)t.pt  )[i-q] :
                                   ((uint32_t *)ptPrev)[BLOCK_SIZE/32-q+i]);
            if (reKey(&t.ki) != TF_SUCCESS)
                FatalError("reKey return during %s test",fname);
            }
        AES_EndSection(&t);
...
#define MAX_ARGS    40
            ...
            switch (toupper(argList[i][1]))
                ...
                case 'F':
                    switch (toupper(argList[i][2]))
                        ...
                        case 'L':   FMT_LOG     =   1;
                                    testCnt     =   0;  break;
                        case 'B':   CLKS_DWORD   =   ~CLKS_DWORD; break;
    ...
    if (testCnt)
    {
        printf( "test count = %u, initializing ... ", testCnt );
